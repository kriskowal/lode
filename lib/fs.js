...
*/
exports.get = get;
function get(url, fs, http, memo) {
    var parsed = URL.parse(url);
    memo = memo || {};
    // returns an fs of the non-fragment portion of the url
    return Q.when(canonical(url, parsed, fs, http), function (url) {
        if (!memo[url])
            memo[url] = fetch(url, fs, http);
        return memo[url];
    }).then(function (fs) {
        var fragment = parsed.hash;
        if (fragment)
            fs = get(fragment.slice(1), fs, http)
        return fs;
    });
}

function fetch(url, fs, http) {
    var parsed = URL.parse(url);
...
function canonical(url, parsed, fs, http) {
    ...
    if (
        ...
        if (!fs)
            return Q.reject("Cannot access the file system via " + url);
        return Q.when(fs.canonical(parsed.pathname || ''), function (path) {
            return URL.resolve('file:/', path);
        });
    } else {
        if (!http)
            return Q.reject("Cannot access the web via " + url);
        return Q.ref(url);
    }
}
