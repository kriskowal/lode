...
require("./weak-map");
var VM = require("vm");
var allPossibleFreeNames = require("./all-possible-free-names").allPossibleFreeNames;
...
var unsafeEval = VM.runInThisContext;
...
function compile(exprSrc, fileName, lineNo) {
    if (dirty) { fail('Initial cleaning failed'); }
    verifyStrictExpression(exprSrc);
    var freeNames = allPossibleFreeNames(exprSrc);
    var wrapperSrc =
        '(function() { ' +
        ...
        // strict code, where this === virtualGlobal
            ...
            '}' +
        '})';
    var wrapper = unsafeEval(wrapperSrc, fileName, lineNo);
    return function(env) {
...
function Module(source, fileName, lineNo) {
}
 ...
 * itself to be.
 */
var FakeFunction = function Function(var_args) {
...
var FakeError = function Error(message) {
    var real = new UnsafeError(message);
    this.message = message;
    this.box = brand.seal(real);
    Object.freeze(this);
};
...
[
    ...
    var SubError = function (message) {
        FakeError.call(this, message);
    };
    SubError.prototype = Object.create(FakeError.prototype);
    SubError.constructor = SubError;
    global[name] = SubError;
});

var solids = WeakMap();

exports.protect = function(root) {
...
console.log('Deleted ' + goodDeletions.sort().join(' '));
...
if (cantNeuter.length >= 1) {
    ...
    console.log("Can't neuter:" + complaint.sort().join('\n'));
}

if (badDeletions.length >= 1) {
    console.log("Can't delete " + badDeletions.sort().join(' '));
} else {
    // We succeeded. Enable safe Function, eval, and compile to work.
    dirty = false;
}

exports.unsealException = function (exception) {
    if (exception.box) {
        return brand.unseal(exception.box);
    } else {
        return exception;
    }
};
...
process.on('uncaughtException', function (error) {
    error = exports.unsealException(error);
    console.log(error && error.stack || error);
    process.exit();
});

exports.boundary = function (callback) {
    try {
        return callback();
    } catch (exception) {
        if (exception.box) {
            exception = brand.unseal(exception.box);
        }
        throw exception;
    }
};
