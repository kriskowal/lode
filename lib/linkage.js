                ...
                memo[got.href] = Q.when(fs, function (fs) {
                    return Q.when(fs.read("package.json"), function (content) {
                        ...
                        loaders = Q.ref(loaders.map(function (loader) {
                            if (loader.compiler) {
                                var pkg = loadPackageLinkage(loader.compiler, suboptions);
                                return {
                                    "extension": loader.extension,
                                    "compiler": pkg.href
                                }
                            } else if (loader.interpreter) {
                                throw new Error("XXX"); // TODO
                            } else {
                                throw new Error("YYY"); // TODO
                            }
                        })
                        .concat([
                            {"extension": ""},
                                    ...
                                    return Q.when(list, function (list) {
                                        ...
                                        list.forEach(function (path) {
                                            ...
                                            candidates[extension] = {
                                                ...
                                                "fs": root.fs,
                                                "path": path,
                                                "packagePath": got.fs.join("/", got.path, root.path, path),
                                                "id": id,
                                                "package": got.href
                                            };
                                        });
                                        ...
                                        for (var i = 0, ii = loaders.length; i < ii; i++) {
                                            ...
                                            if (candidates[loader.extension]) {
                                                ...
                                                if (extensions.length > 1) {
                                                    options.warn("Multiple candidates for module " + JSON.stringify(id) + " in package " + JSON.stringify(got.href) + " including " + JSON.stringify(extensions) + ". Chose " + JSON.stringify(election.path) + ".");
                                                }
                                                if (loader.compiler || loader.interpreter)
