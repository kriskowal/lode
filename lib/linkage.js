...
function findIdForPath(linkage, path) {
    ...
    throw new Error(
        "Cannot find module for path " + JSON.stringify(path) +
...
function loadPackageLinkage(dependency, options) {
    ...
    if (dependency.system !== undefined) {
        ...
        capabilities[dependency.capability] = true;
        return linkage[href] = {
            "href": href,
            "ids": [""],
            "lib": {
...
function loadLanguageInfo(config) {
    ...
    return Q.shallow(languages.map(function (loader) {
        if (loader.compiler) {
            return loader;
        } else {
...
function loadMappingInfo(config, options) {
    ...
    Object.keys(mappings).map(function (key) {
        var mapping = mappings[key] = Dependency(mappings[key]);
        var pkg = loadPackageLinkage(mapping, options);
        map[key] = pkg;
    });
    ...
    var lib = Q.when(config, function (config) {
        ...
        .then(function (roots) {
            ...
            if (config.main) {
                ...
                var path = got.fs.join("/", got.path, config.main);
                tree[""] = tree[""] || {};
                tree[""][extension] = {
                    "href": got.href.replace(/\/$/, "") + path,
                    "fs": fs,
...
function configure(config, pkg, options) {
    ...
    if (Array.isArray(dependencies)) {
        ...
        // Add a .node language
        //
        /* TODO support .node as a module language
        ...
        */
        ...
        Object.keys(dependencies).forEach(function (dependency) {
            ...
            var predicate = NPM.VersionPredicate(version);
            mappings[dependency] = Q.when(getNpmPath(options), function (npmPath) {
                var path = FS.join(npmPath, dependency);
                var using;
                return Q.when(FS.list(path), function (versions) {
                    versions.sort(NPM.compareVersions);
                    ...
                    if (!using) {
                        throw new Error("Can't execute package because it depends on an NPM package that isn't installed.");
                    } else {
...
function compileModule(module, pkg, linkage, options) {
    ...
    if (!module.loader) {
        ...
        return Q.when(module.content, function (content) {
            var compiler = requireDependency(module.loader.compiler, {
...
function linkPackage(pkg, linkage, requires) {
    ...
    Object.keys(pkg.lib).forEach(function (id) {
        var module = pkg.lib[id];
        factories[id] = linkModule(module, requires, PkgModule);
    });
...
function linkModule(module, requires, Module) {
    if (module.external) {
        return function () {
        ...
        return function () {
            throw new Error("capability not ready");
            // TODO
        };
    } else {
        throw new Error(
...
function verifyPackage(pkg) {
    // TODO, compare hashes of includes, loaders, and mappings
}
