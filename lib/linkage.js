            ...
            if (!memo[got.href]) {
                ...
                memo[got.href] = Q.when(fs, function (fs) {
                    ...
                    return Q.when(config, function (config) {
                        ...
                        var loaders = loadLoaderInfo(config);
...
function loadLoaderInfo(config) {
    var loaders = config.loaders || [];
    return Q.shallow(loaders.map(function (loader) {
        if (loader.compiler) {
            return loader;
        } else {
            throw new Error("Loaders must have a compiler property.");
        }
    })
    .concat([
        {"extension": ""},
...
function loadMappingInfo(config, options) {
    ...
    Object.keys(mappings).map(function (key) {
        ...
        map[key] = loadPackageLinkage(mapping, options);
    });
...
function loadLibInfo(config, options, fs, got, roots, loaders, mappings) {
...
function configure(config, pkg, options) {
    ...
    if (config.overlays) {
        (options.engines || []).forEach(function (engine) {
            update(config, config.overlays[engine] || {});
            config[engine] = true;
        });
